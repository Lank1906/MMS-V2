...
exports.getRoomTypes = (page, limit, search, callback) => {
  ...
  db.query(sql, [`%${search}%`, limit, offset], (err, results) => {
    ...
    db.query(
      'SELECT COUNT(*) AS total FROM RoomTypes WHERE name LIKE ? AND is_active = TRUE',
      [`%${search}%`],
      (err2, countResult) => {
...
exports.createRoomType = (data, callback) => {
  ...
  db.query(sql, [name, description, max_occupants, rent_price, electricity_price, water_price, charge_type], (err, result) => {
    if (err) return callback(err);
